  ...
  try {
    const segments = await Promise.all(
      segmentVideos.map(async (segment, index) => {
        const uniqueFileName = `segment_${segment.index}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}.mp4`;
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          ({ data: uploadData, error: uploadError } = await supabase.storage
            .from('avatars')
            .upload(`input/${uniqueFileName}`, blob, {
              contentType: 'video/mp4',
            }));
        ...
        const { data: urlData } = supabase.storage
          .from('avatars')
          .getPublicUrl(`input/${uniqueFileName}`);
        ...
        console.log(`Prepared segment ${index + 1}:`, { videoPath, uniqueFileName });
        ...
        return {
          ...
          uniqueId: uniqueFileName, // Add unique identifier
        };
      })
    );

    console.log('Sending segments to /api/bulkrender:', segments);
    ...
    setMessage(
      `Rendering initiated for ${workflowIds.length} segments. Workflow IDs: ${workflowIds.map(id => id.workflowRunId).join(', ')}`
    );

    setWorkflowData({ workflowIds, segmentPaths: segments.map((s) => s.videoUrls[0].src) });
    console.log('Workflow data set:', workflowData);
  } catch (error) {
